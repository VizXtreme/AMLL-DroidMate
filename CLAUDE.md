...
- **`LyricsRepository`** — Multi-source lyrics search/fetch. Parallel search across QQ Music (LRC), Netease (YRC/LRC), Kugou (KRC), and AMLL TTML DB. Includes sophisticated title/artist matching with Levenshtein distance, accent normalization, and version-keyword detection. All results are normalized to TTML via `TTMLConverter`.
- **`UnifiedLyricsParser`** — Format-detecting parser that dispatches to format-specific parsers (LRC, Enhanced LRC, KRC, YRC, TTML). All formats produce `UnifiedLyrics` (common data model) with `LyricLine`s containing text, translation, transliteration, per-word timing, and structural markers (isBG, isDuet, agent).
