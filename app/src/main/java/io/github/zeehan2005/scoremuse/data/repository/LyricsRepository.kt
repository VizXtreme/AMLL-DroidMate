...
open class LyricsRepository(
     ...
     * 从 QQ 音乐获取歌词内容
     */
    open suspend fun getQQMusicLyrics(songMid: String, title: String? = null, artist: String? = null): UnifiedLyrics? {
        return try {
            val (mid, numericId) = parseQqSongIds(songMid)

            val fallbackMid = mid ?: songMid

            // 1) lyric_download.fcg 作为主歌词获取方式（需 numeric ID）
            //    它可能同时返回翻译(contentts)/音译(contentroma)，也可能不返回
            val primaryResult = if (numericId != null) {
                Timber.i("[LyricsRepository] Trying lyric_download.fcg as primary with songID=$numericId")
                getQQMusicLyricsViaLyricDownload(numericId, title, artist)
            } else null

            val hasPrimaryTrans = primaryResult?.lines?.any {
                it.translation?.isNotBlank() == true
            } == true
            val hasPrimaryRoma = primaryResult?.lines?.any {
                it.transliteration?.isNotBlank() == true
            } == true

            if (primaryResult != null) {
                if (hasPrimaryTrans && hasPrimaryRoma) {
                    Timber.i("[LyricsRepository] lyric_download.fcg succeeded with full trans/roma, lines=${primaryResult.lines.size}")
                    return primaryResult
                }
                Timber.i("[LyricsRepository] lyric_download.fcg has main lyrics but missing trans($hasPrimaryTrans)/roma($hasPrimaryRoma), supplementing from musicu.fcg")
            }

            // 2) 通过 musicu.fcg 获取翻译/音译（trans/roma），同时也可作为主歌词的回退来源
            ...
            if (!lyricContent.isNullOrBlank()) {
                Timber.d("[LyricsRepository] QQ Music musicu.fcg lyric length=${lyricContent.length}")
            }
            ...
            // 3) 决定主歌词来源：优先用 lyric_download.fcg 结果，回退到 musicu.fcg
            val mainLyrics = primaryResult ?: run {
                if (lyricContent.isNullOrBlank()) {
                    Timber.i("[LyricsRepository] No lyrics content from QQ Music for: $fallbackMid")
                    return null
                }
                Timber.i("[LyricsRepository] Using lyrics from musicu.fcg")
                ...
                if (decoded.length < 5) {
                    Timber.e("[LyricsRepository] QQ Music LRC content too short: '$decoded'")
                    return null
                }
            ...
            // 4) 从 musicu.fcg 解析翻译/音译（补充 lyric_download.fcg 缺失的部分）
            val musicuTrans = transContent
                ?.takeIf { it.isNotBlank() && it != "0" && it.length >= 5 }
                ?.let {
                    runCatching {
                        val decoded = decodeQqLyricPayload(it)
                        io.github.zeehan2005.scoremuse.data.parser.LrcParser.parse(decoded)
                    }.getOrNull()
                }

            val musicuRoma = romaContent
                ?.takeIf { it.isNotBlank() && it != "0" && it.length >= 5 }
                ?.let {
                    runCatching {
                        val decoded = decodeQqLyricPayload(it)
                        UnifiedLyricsParser.parse(decoded, processMetadata = processMetadata)?.lines
                    }.getOrNull()
                }

            // 5) 合并：只把 mainLines 中缺失的翻译/音译从 musicu.fcg 补充进来
            val merged = mergeLyricLines(
                mainLyrics.lines,
                if (hasPrimaryTrans) null else musicuTrans,
                if (hasPrimaryRoma) null else musicuRoma
            )
    ...
    private suspend fun getQQMusicLyricsViaLyricDownload(
        ...
        return try {
            ...
            if (mainEncrypted.isNullOrBlank()) {
                Timber.w("[LyricsRepository] lyric_download.fcg returned no <content> CDATA")
                return null
            }
            ...
            Timber.i("[LyricsRepository] QQ lyric_download failed $e")
            null
        }
    }
     ...
     * 从网易云音乐获取歌词内容
     */
    open suspend fun getNeteaseLyrics(songId: String, title: String? = null, artist: String? = null): UnifiedLyrics? {
        return try {
            val payload = buildJsonObject {
                put("id", songId)
                put("cp", "false")
     ...
     */
    open suspend fun getKugouLyrics(idOrHash: String, title: String? = null, artist: String? = null): UnifiedLyrics? {
        return try {
            // 基于 Unilyric 的实现
    ...
    internal fun compareArtists(artists1: String?, artists2: String?): ArtistMatchType? {
        ...
        // This helps handle cases where ScoreMuse returns multiple alternate artist spellings;
    ...
    private fun mergeNeteaseAuxiliaryLines(
        ...
        val translationLines = translationContent
            ?.takeIf { it.isNotBlank() }
            ?.let { io.github.zeehan2005.scoremuse.data.parser.LrcParser.parse(it) }
     ...
     */
    open suspend fun getAMLLLyrics(
        songId: String,
        title: String? = null,
        artist: String? = null
    ): UnifiedLyrics? {
        ...
        val qqMusicJob = launch {
            ...
            list.forEach { result ->
                amllProbeScope.launch outer@{
                    ...
                    val channel = Channel<Pair<String, UnifiedLyrics>>(capacity = 1)
                    val jobs = segments.map { seg ->
                        amllProbeScope.launch {
                            ...
                            val amllResult = runCatching {
                                getAMLLLyrics(prefixId, title = result.title, artist = result.artist)
                            }.getOrNull()
                            if (amllResult != null) {
    ...
    protected open suspend fun getCachedLyrics(title: String, artist: String): LyricsResult? {
        ...
        val parsed = entry.xmlContent.takeIf { it.isNotBlank() }
    ...
    suspend fun getLyrics(
        ...
        return try {
            ...
            val lyrics = when (normalizedProvider) {
                "amll" -> getAMLLLyrics(songId, title, artist)
    ...
    open suspend fun getLyricsFeatures(
        ...
        val normalizedProvider = provider.lowercase()
        val lyrics: UnifiedLyrics? = when (normalizedProvider) {
            "amll" -> getAMLLLyrics(songId, title, artist)
            "netease", "ncm" -> getNeteaseLyrics(songId, title, artist)
            "qq", "qqmusic" -> getQQMusicLyrics(songId, title, artist)
            "kugou" -> getKugouLyrics(songId, title, artist)
    ...
    companion object {
        ...
        fun parseTTML(
            ...
            return try {
                Timber.d("[BG-LYRICS-DEBUG] LyricsRepository.parseTTML input: length=${xmlContent.length}")
                // 使用 TTML 解析器
                Timber.d("[TTMLParser] Detected TTML format, using TTMLParser")
                val parsedLyrics = TTMLParser.parse(xmlContent)
                
                if (parsedLyrics.lines.isEmpty()) {
                    Timber.e("[LyricsRepository] Failed to parse lyrics content")
                    return null
                }
                ...
                // 保存原始内容到 rawTtml 字段，用于后续 WebSocket 发送
                UnifiedLyrics(
                    metadata = metadata,
                ...
                Timber.e("[LyricsRepository] Error parsing lyrics $e")
                null
            }
        }
